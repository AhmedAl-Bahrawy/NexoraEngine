...
description: Common utility functions for validation, formatting, and rate limiting to support application development. Reduces boilerplate code with standardized utilities.
license: MIT
...
- Use validators for input validation before database operations
- Use formatters for displaying data in UI
- Use rate limiter for API protection and abuse prevention

## When NOT to Use

- Don't use for complex validation (use Zod schemas instead)
- Don't use for persistent storage (rate limiter is in-memory)
- Don't bypass rate limiting for protected endpoints
